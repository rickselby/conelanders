...
class DriverStandings implements StandingsInterface
{
    ...
    /**
     * @var \App\Services\RallyCross\DriverStandings
     */
     ...
     * @var string
     */
    protected $cacheKey = 'rallycross.driver-standings.';

    public function __construct(Repository $cache, \App\Services\RallyCross\DriverStandings $driverStandingsService)
    {
        $this->cache = $cache;
    ...
    public function heatsSummary(RxEvent $event)
    {
        return $this->eventCache(
            $event,
            $this->cacheKey . 'heatSummary.' . $event->id,
            function () use ($event) {
                return $this->driverStandingsService->heatsSummary($event);
            }
        );
    }
