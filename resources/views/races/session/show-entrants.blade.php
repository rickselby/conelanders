    ...
    <div class="panel-collapse collapse {{ count($session->entrants) ? '' : 'in' }}" id="entrants" role="tabpanel">
        <div class="panel-body">
            @if (count($session->entrants))
                {!! Form::open(['route' => ['races.championship.event.session.entrants.update', $session->event->championship, $session->event, $session], 'class' => 'form-horizontal']) !!}
                            ...
                            <td>
                                @if ($entrant->canBeDeleted())
                                    <a class="btn btn-small btn-danger"
                                       href="{{ route('races.championship.event.session.entrants.destroy', [$session->event->championship, $session->event, $session, $entrant]) }}">
                                        Delete
                                    </a>
                                @endif
