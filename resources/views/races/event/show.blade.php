...
@section('content')
    ...
    {!! Form::open(['route' => ['races.championship.event.destroy', $event->championship, $event], 'method' => 'delete', 'class' => 'form-inline']) !!}
        <a class="btn btn-small btn-warning"
           href="{{ route('races.championship.event.edit', [$event->championship, $event]) }}">Edit Event</a>
    ...
    <p>
        <a class="btn btn-small btn-info"
           href="{{ route('races.championship.event.session.create', [$event->championship, $event]) }}">Add a new session</a>
    </p>

    @if (count($event->sessions) == 0 && count($otherEvents))
        {!! Form::open(['route' => ['races.championship.event.copy-sessions', $event->championship, $event], 'class' => 'form-horizontal']) !!}
        ...
        @forelse($event->sessions AS $session)
            <tr class="{{ $session->canBeReleased() ? '' : 'info' }}" data-id="{{ $session->id }}">
                <td>
                    ...
                    <a href="{{ route('races.championship.event.session.show', [$event->championship, $event, $session]) }}">
                        {{ $session->name }}
                    </a>
                </td>
                <td>
        ...
        $("#sessions").sortable({
            store: {
                ...
                set: function(sortable) {
                    ...
                    $.post(
                        "{{ route('races.championship.event.sort-sessions', [$event->championship, $event]) }}",
                        {
