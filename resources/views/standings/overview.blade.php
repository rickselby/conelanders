...
@section('header')
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('standings.championship', [$system->id, $championship->id]) }}">{{ $championship->name }}</a></li>
            ...
            @foreach($seasons AS $season)
                @foreach($season->events AS $event)
                    <th colspan="{{ count($event->stages) + 1 }}" data-sorter="false" class="text-center">
                        <a href="{{ route('standings.event', [$system->id, $championship->id, $season->id, $event->id]) }}" class="tablesorter-noSort">
                            {{ $event->name }}
                        </a>
                    </th>
                @endforeach
                ...
                @foreach($season->events AS $event)
                    @foreach($event->stages AS $stage)
                        <th data-sortInitialOrder="desc">
                            <a href="{{ route('standings.stage', [$system->id, $championship->id, $season->id, $event->id, $stage->id]) }}" class="tablesorter-noSort">
                                {{ $stage->order }}
                            </a>
                        </th>
                    @endforeach
